...
**機材面の裏付け** (2026-05 / Slice 8): この体験品質を支える物理的な機材として、開発者は **ENRO 電気ピザ窯**を導入。家庭オーブンの 250〜300℃ では到達できない **400〜450℃ / 90〜120 秒** の高温短時間焼成と、電源さえあれば屋内・屋外・レンタルキッチンへ持ち出せる**ポータブル性**が、「ホストが本気で出せる一枚」と「持ち寄り型ピザパーティ」を物理的に可能にしている。本アプリのレシピ生成は ENRO を**推奨機材**として最適化しつつ、家庭オーブン用プロファイルも設定で切り替え可能 (§3.2 F13, F14)。
...
| **NEW-1** | **家庭オーブンでアプリ通りに作っても "美味しくない"** | 250〜300°C で焼くしかない | 「画面の通りに作ったのに美味しくない」体験で離脱・期待ギャップ |
| **NEW-2** | **何を揃えればこのアプリ体験を再現できるか分からない** | 暗黙の前提機材があるか判別不能 | 機材投資の正解が示されない / 初心者の参入障壁 |

> **C2 と C7 は新仕様で新たに焦点化した課題。** C2 はウィザード型 UX 自体への反省、C7 はフィードバック活用を「学習」だけでなく「発想拡張」にまで広げた本仕様独自の課題設定。
> **NEW-1 / NEW-2 は Slice 8 (2026-05) で導入**。レシピは特定機材を前提に最適化されるため、機材によって体験品質が大きく変わる事実を**プロダクト体験に明示的に統合** (F13 機材プロファイル / F14 機材ガイドページ)。
...
| F13 | **機材プロファイル** (Slice 8) | 詳細レシピ生成時の焼成温度・時間・生地厚・水分指示を機材に合わせて切替。`enro_450c_90s` (推奨・デフォルト) / `home_oven_280c_10m` (補足) の 2 種。Tap2 ヘッダ右の独立セレクタ + HeaderRow Dropdown 内の現在プロファイル表示で随時切替可能 | NEW-1 |
| F14 | **機材ガイドページ** (`/equipment` / Slice 8) | ENRO 電気ピザ窯を推奨機材として紹介する独立 LP。Hero / 開発者の声 / 3 体験変化 (高温・ポータブル運用・価格) / 5 ステップ + 比較表 / YouTube / 家庭オーブン補足 / アフィリエイト透明性注記。楽天アフィリエイトリンクは `rel="sponsored"` + 開示文を伴って提示 | NEW-2 |
...
| 機材プロファイル (Slice 8 / F13) | **端末ローカル (localStorage)** | クラウド同期 |
| レシピ保存 | クラウド(Google ログイン認証時のみ) | 同左 + 公開・共有 |
| フィードバックデータ | クラウド(認証時のみ) | 同左 + 集計可視化 |

**MVP の方針**: 認証なしでも Quick Tap 体験は完全に成立させる。地元選択の永続化(F1) と食材選択 + 機材プロファイル (F13) は localStorage で完結し、Google ログインは**レシピ保存・フィードバック記録を行いたいユーザーのみオプトイン**。これにより「サクッと試す」初見ユーザーの体験ハードルを最低化する。

### 5.5 アフィリエイト・透明性 (Slice 8 / F14 由来)

- **採用**: 楽天アフィリエイトを `/equipment` ガイドページ内の ENRO 関連リンクで使用
- **開示**: すべての該当リンクに `rel="sponsored noopener noreferrer"` を付与し、ページ末尾に**「楽天アフィリエイトを含む / 開発者は ENRO の 1 ユーザとして推奨しており、メーカー・販売店との提携・PR 案件ではない」**旨の文言を常時表示
...
### US-9: 機材を揃える前に正解を知る (Slice 8)
> これからホスト体験を始める初心者として、どの機材を揃えれば本アプリのレシピが本当に美味しく仕上がるかを知りたい。なぜなら、「家で作ったけど美味しくない」を回避したいし、機材選びの正解を一発で知りたいからだ。

### US-10: 持ち運んでピザパーティを開く (Slice 8)
> ホストとして、ENRO のポータブル性を活かして、レンタルキッチンや友人宅でも「地元食材ピザパーティ」を開きたい。なぜなら、自宅以外の場でも、その場で焼きたてを振る舞う体験こそがピザパーティの本質だからだ。

### US-11: 機材なしでも試してみたい (Slice 8 / 補足)
> 機材を買うつもりはまだないユーザとして、手持ちの家庭用オーブンでもそれなりに作れる代替手順を知りたい。なぜなら、機材投資の前に "この体験は自分に合うか" を試したいからだ。
...
### AC-9: 機材プロファイル + 機材ガイド (Slice 8 / US-9, US-10, US-11 / F13, F14)
- **AC-9-1**: 初期状態で生成されたレシピは焼成温度 **400〜450°C / 焼成時間 60〜180 秒** の範囲で出力される (ENRO 前提)
- **AC-9-2**: 機材プロファイルを「家庭用オーブン」に切替えると、生成レシピの焼成温度は **250〜300°C / 焼成時間 5〜15 分** の範囲に変わる
- **AC-9-3**: 機材プロファイル選択は localStorage に永続化され、リロード後も維持される。HeaderRow Dropdown 内に「現在: 〜」表示があり、Tap2 ヘッダ右の独立セレクタからいつでも変更可能
