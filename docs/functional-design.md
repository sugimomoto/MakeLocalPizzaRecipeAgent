...
| `mlpr.guestSessionId.v1` | 認証なし時の匿名セッション識別子。Slice 9 でレートリミット key、Slice 10 で共有所有者識別にも利用 | 永続(認証後はクラウド側 user_id に紐付け) |
| `mlpr.ovenProfile.v1` | 選択中の機材プロファイル (`enro_450c_90s` / `home_oven_280c_10m`)。Slice 8 | 永続(明示変更まで) |
...
| POST | `/api/share` | 共有レシピ発行 (Slice 10) | レシピ snapshot 一式 | `{ shareId, url }` | 任意 (ゲスト可) |

> **Slice 8 追補**: 詳細生成 (`POST /api/recipes/{candidateId}`) は任意の `ovenProfile`
> (`enro_450c_90s` / `home_oven_280c_10m`) を受け取り、焼成温度・時間・生地指示を切替える
> (省略時は ENRO に解決)。詳細は §11.1 を参照。
> **Slice 10 追補**: `/share/[shareId]` は認証不要の公開 **ページ** (Server Component) で、
> `generateMetadata` が OGP / Twitter Card を動的生成する。詳細は §11.3 を参照。
...
**Slice 9 で実装済み** (アプリ層レートリミット)。判定単位 (rate-limit key) は
`auth:{uid}` > `guest:{guestSessionId}` > `ip:{XFF 先頭}` > `anonymous` の優先順。
UTC 1 時間 (`YYYYMMDDHH`) を hour bucket とし、`rate_limits/{docId}` を Firebase Admin SDK で
atomic にインクリメント。超過時は `429` + `Retry-After`(= 次の時刻境界までの秒数)。
ドキュメントは `expiresAt`(bucket + 2h)で Firestore TTL により自動削除。

| ルート | 上限 | 根拠 |
| --- | --- | --- |
| `POST /api/quicktap/sessions` | 10 / 時 | Gemini Flash × 3 ≒ $0.003/req |
| `POST /api/quicktap/sessions/{id}/reroll` | 10 / 時 | 候補と同コスト |
| `POST /api/recipes/{candidateId}` | 5 / 時 | Gemini + Imagen ≒ $0.04/req(最も厳しめ) |
| `POST /api/share` | 5 / 時 | LLM コストなし。`shared_recipes` 量産の悪用防止が主目的 |

- 楽天 API への内部キャッシュ更新: 1 req/秒(楽天側制限の遵守、レートリミットとは別系統)
...
> Slice 7 以降に追加された機能を、本書の既存章を補足する形でまとめる。
> 各 Slice の詳細なステアリングは `.steering/` 配下を参照。

### 11.1 機材プロファイル / 機材ガイド (Slice 8)

- **目的**: レシピは焼成機材で最適温度・時間・生地が変わる。焼成機材の前提を
  「機材プロファイル」として生成プロンプトに注入し、手元の機材で再現できるレシピにする。
- **プロファイル**: `enro_450c_90s`(ENRO 電気ピザ窯・400〜450°C・90秒・**デフォルト/推奨**) と
  `home_oven_280c_10m`(家庭用オーブン・250〜300°C・8〜15分・補足) の 2 値。
- **データフロー**: 端末は `localStorage`(`mlpr.ovenProfile.v1`)に選択を保持 →
  詳細生成リクエストの `ovenProfile` に載せる → Agent 側 `resolve_oven_profile` が未指定/未知 ID を
  ENRO に丸めてプロンプトへ反映(`meta.bakingTemp` / `meta.duration` / 生地厚 / 水分指示を切替)。
- **機材ガイド**: `/equipment` は ENRO を推奨機材として紹介する独立 LP
  (Hero / 開発者の声 / 体験変化 / ステップ + 比較表 / YouTube / 家庭オーブン補足 /
  アフィリエイト透明性注記)。楽天アフィリエイトは `rel="sponsored"` + 末尾の明示注記を必須とする。

### 11.2 アプリ層レートリミット (Slice 9)

- **目的**: ボット連投で Vertex AI / Imagen のコストが爆発するのを防ぐ。
- **方式**: `withAuthOptional(withRateLimit(...))` ミドルウェアで公開 API をラップ。
  判定単位 (rate-limit key) は `auth:{uid}` > `guest:{guestSessionId}` > `ip:{XFF 先頭}` >
  `anonymous` の優先順(NAT 巻き添え回避のため guest/auth を優先)。
- **カウンタ**: UTC 1 時間 (`YYYYMMDDHH`) を hour bucket とし、`rate_limits/{docId}` を
  Firebase Admin SDK で atomic にインクリメント。`docId = bucket × key × routeKey`。
  超過時は `429` + `Retry-After`(次の時刻境界までの秒数)。`expiresAt`(bucket + 2h)で
  Firestore TTL により自動削除。上限値は §6.4 の表を参照。
- **Slice 9.1**: client は `apiFetch` ラッパ経由で `x-mlpr-guest-session-id` ヘッダを自動付与。
  429 受信時は Toast で残り時間を案内。あわせて GA4 `trackEvent` を導入。

### 11.3 共有リンク (Slice 10)

- **目的**: 詳細レシピ(ストーリー + 仕上がり画像 + 地元食材)を 1 枚の公開ページとして
  SNS(主に X)へ共有し、未ログイン段階から自然拡散を起こす。
- **発行**: `POST /api/share`(認証ユーザ / ゲスト両対応)がレシピ snapshot を受け取り
  `shared_recipes/{shareId}` へ書き込み `{ shareId, url }` を返す。同 `(owner, candidateId)` からの
  2 回目以降は `share_index/{indexKey}` で**べき等**に既存 `shareId` を再利用。`shareId` は UUID v4。
- **公開ページ**: `/share/[shareId]` は認証不要の Server Component。Firestore をサーバから読み、
  該当なしは `notFound()` で 404。`generateMetadata` が OGP / `twitter:card=summary_large_image` を
  動的生成し、画像入り大型カードで展開される。
- **拡散導線**: 確定後に X Web Intent(`https://x.com/intent/post?text=...&url=...`)を新タブで開く。
  投稿テキストは 280 字に収まるよう client 側テンプレートで切り詰める。
- **データモデル**: `shared_recipes/{shareId}`(public read / Admin SDK のみ write) と
  `share_index/{indexKey}`(Admin SDK のみ R/W)。UID は URL / OGP に露出させない。
...
| 2026-05-24 | 1.2 | Slice 7 完了 (v0.7.0)。新画面 2 本 (`/feedback/[id]` + `/journal`)、統一 HeaderRow + Dropdown、FurusatoMark ブランドマーク、SavedRecipe に `feedback` field 追加、Firestore drafts subcollection (3 秒 debounce auto-save)、保存帳 ⇄ 振り返り帳の対と「作った」サブバッジ。詳細は `.steering/20260524-slice7-feedback/` を参照。 |
| 2026-06-13 | 1.3 | Slice 8〜10 を反映。§11 機能追補(機材プロファイル / アプリ層レートリミット / 共有リンク)を新設し、§6.1 に `POST /api/share`、§6.4 を実装済みレートリミット値に更新、§3.4 に `mlpr.ovenProfile.v1` を追加。詳細は `.steering/20260525-enro-equipment-guide/` `20260530-slice9-rate-limit/` `20260606-share-link/` を参照。 |
