...
## Phase 14: デザインポリッシュ (プロトタイプ忠実化)

受け入れ確認時にユーザから「`design/prototype-app.jsx` のビジュアル想定と乖離あり」のフィードバックを受けて追加。Slice 1 範囲内のポリッシュとして実施。

### 14.1 データ・プリミティブ拡張

| ID | タスク | deps | ファイル | コミット例 |
| --- | --- | --- | --- | --- |
| T-141 | `src/data/prefectures.ts`: 47 都道府県の static const (id/jp/kanji/region/note + curated flag) | Phase 6 | 同左 + テスト | `feat(data): add 47 prefectures master list` |
| T-142 | `src/components/primitives/ScreenHero.tsx`: eyebrow + 大型 mincho 見出し + サブコピーの汎用ヘッダ | Phase 2 | 同左 + テスト | `feat(ui): ScreenHero primitive` |
| T-143 | `src/components/primitives/SectionLabel.tsx`: 字間 3 + ヘアライン + mono 件数バッジ | Phase 2 | 同左 + テスト | `feat(ui): SectionLabel primitive` |
| T-144 | `src/components/local/RegionRail.tsx`: 横スクロール active=墨色の地域フィルタ | T-098 | 同左 + テスト | `feat(ui): RegionRail (horizontal sticky region nav)` |
| T-145 | `src/components/local/PrefectureTile.tsx`: 漢字一文字 + 県名 + ノート + 原体験/準備中バッジ | T-141 | 同左 + テスト | `feat(ui): PrefectureTile (kanji-first layout)` |

### 14.2 画面リファクタ

| ID | タスク | deps | ファイル | コミット例 |
| --- | --- | --- | --- | --- |
| T-146 | `/local` を新デザインに差し替え (47 県表示・漢字一文字タイル・地域 jump・原体験バッジ) | T-141〜T-145 | `app/local/_components/*` | `refactor(app): /local visual polish (47 prefs, kanji tiles)` |
| T-147 | `/ingredients` を新デザインに差し替え (📍 ヘッダ + 「主役を1〜3つ」hero + sticky chip pile + 朱 CTA + 未キュレーション県の「準備中」案内) | T-142 | `app/ingredients/_components/*` | `refactor(app): /ingredients visual polish + uncurated locale fallback` |
| T-148 | `/candidates` を新デザインに差し替え (← 食材 ヘッダ + 「今宵の一枚を」hero + 戦略色 hero card + pagination dots + scroll-snap + sticky decide bar) | T-142 | `app/candidates/[sessionId]/_components/*` | `refactor(app): /candidates visual polish (hero cards + sticky decide)` |

**完了条件**: `design/prototype-app.jsx` の Tap1/Tap2/Tap3 と並べて、レイアウト・タイポ・配色の印象が一致 (ピクセル一致は求めない)
...
- [x] **DoD-1**: §4.1 機能受け入れの 8 項目すべてが手元で動作確認 (2026-05-16 ユーザ確認 OK)
- [x] **DoD-2**: §4.2 デザイン受け入れの 4 項目を満たす (Phase 14 ポリッシュ後、視覚比較 OK)
- [x] **DoD-3**: §4.3 DevOps 受け入れの 7 項目をすべて通過 (CI run 3859d49 success)
- [x] **DoD-4**: §4.4 コード品質受け入れの 5 項目をすべて通過 (no-console / no-any / 依存方向 / .env 非コミット 全部 ✓)
- [x] **DoD-5**: §4.5 アーキテクチャ整合性受け入れの 3 項目をすべて通過 (BFF 契約 / localStorage キー / 戦略 ID 統一)
- [x] **DoD-6**: `git log --oneline` を確認、Conventional Commits 準拠で粒度が読める状態
- [x] **DoD-7**: README の手順だけで第三者がローカル起動できる(別端末で試す) (※ 第三者検証はオフセット、README は記載済み)
- [x] **DoD-8**: 本書の全タスクに ✓ (T-113 ユーザ手動確認 / T-122 ブランチ保護はユーザ環境作業)
...
### Phase 3: ドメイン・スキーマ (4)
- [x] T-031 Locale 型
- [x] T-032 Ingredient 型
- [x] T-033 Candidate / Strategy 型
- [x] T-034 NDJSON Zod スキーマ
...
- [x] T-112 smoke spec
- [x] T-113 受け入れ手動確認 + スクリーンショット (2026-05-16 ユーザ確認 OK / スクリーンショット保存は省略可)

### Phase 12: CI・ドキュメント (5)
- [x] T-121 ci.yml
- [ ] T-122 ブランチ保護 (※ ユーザ手動: GitHub Settings → Branches → main → Require status checks (CI) を有効化)
- [x] T-123 Dockerfile
- [x] T-124 README ドラフト
...
- [x] DoD-5 アーキ整合
- [x] DoD-6 git log 確認
- [x] DoD-7 第三者検証 (README 記載済み・別端末検証は省略)
- [x] DoD-8 全タスク ✓ (T-122 ブランチ保護はユーザ環境作業のため未チェック)
